package com.portal.common.contact.view;
...
public class SideBar extends View {
    ...
    public static String[] INDEX_STRING = {"0","A", "B", "C", "D", "E", "F", "G", "H", "I",
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < letterList.size(); i++) {
            ...
            // 选中的状态
            if (i == choose) {
                paint.setColor(Color.parseColor("#325470"));
                paint.setFakeBoldText(true);
            }
            // x坐标等于中间-字符串宽度的一半.
            float xPos = width / 2 - paint.measureText(letterList.get(i).equals("0")?"热":letterList.get(i)) / 2;
            float yPos = singleHeight * i + singleHeight / 2;
            canvas.drawText(letterList.get(i).equals("0")?"热":letterList.get(i), xPos, yPos, paint);
            paint.reset();// 重置画笔
        }
    }
...
//                setBackgroundResource(R.drawable.bg_sidebar);
                if (oldChoose != c) {
                    if (c >= 0 && c < letterList.size()) {
                        ...
                        if (mTextDialog != null) {
                            mTextDialog.setText(letterList.get(c).equals("0")?"热":letterList.get(c));
